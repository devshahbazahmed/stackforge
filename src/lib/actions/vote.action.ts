...
export async function createVote(params: CreateVoteParams): Promise<ActionResponse> {
  ...
  if (!userId) return handleError(new UnauthorizedError("Unauthorized")) as ErrorResponse;
...
export async function hasVoted(params: HasVotedParams): Promise<ActionResponse<HasVotedResponse>> {
  ...
  try {
    ...
    return {
      success: true,
      data: { hasUpvoted: vote.voteType === "updvote", hasDownvoted: vote.voteType === "downvote" },
    };
  } catch (error) {
    return handleError(error) as ErrorResponse;
  }
